...
public class Grafo {
    ...
    public ArrayList<Vertice> vizinhos(Vertice v){
        ...
        for(int i = 0; i < numVertices; i++){
            for(int j = 0; j < numArestas;j++){
                if(arestas[j].getOValor() != arestas[j].getDValor()){

                    if(v.getValor() == arestas[j].getOValor()){
                        vizinhos.add(arestas[j].getDestino());
                    }else if(v.getValor() == arestas[j].getDValor()){
    ...
    public String gerarRelatorioDeRotas(){
        ...
        for(int i = 0; i < vertices.length; i++){
            for (int j = 0; j< vertices.length-1;j++){
                if(i != j){
                    rota += "Caminho de: "+ vertices[i].getValor()+ " para "+ vertices[j].getValor()+" é:"+
                            "\n"+ rotaDeReciclagemString(vertices[i],vertices[j])+"\n";
                }
    ...
    public boolean eVizinho(Vertice v1, Vertice v2) {
        for (int i = 0; i < numArestas; i++) {
            if (v1.getValor() == arestas[i].getOValor() && v2.getValor() == arestas[i].getDValor()) {
                return true;
            } else if (v1.getValor() == arestas[i].getDValor() && v2.getValor() == arestas[i].getOValor()) {
                return true;
    ...
    public Aresta[] encontrarAresta(Vertice v1, Vertice v2) {
        ...
        for (int i = 0; i < this.arestas.length; i++) {
            ...
            if (this.arestas[i].getOValor() == v1.getValor() && this.arestas[i].getDValor() == v2.getValor()) {
                //adiciona se tiver
                arestas1.add(this.arestas[i]);
            } else if (this.arestas[i].getOValor() ==  v2.getValor() && this.arestas[i].getDValor() == v1.getValor()) {
                //adiciona se tiver
                arestas1.add(this.arestas[i]);
            }
        }
